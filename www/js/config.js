(function () {
    /* global require, monaco, jsyaml */
    const wrapper = document.querySelector('.editor-wrapper');
    let container = document.getElementById('config');
    const textarea = document.getElementById('config-textarea');
    const saveBtn = document.getElementById('save');
    const suggestBtn = document.getElementById('suggest');
    const btnLoadMonaco = document.getElementById('btn-load-monaco');
    ...
    // Textarea Tab & Auto-indent handlers
    if (textarea) {
        textarea.addEventListener('keydown', (e) => {
            if (e.key === 'Tab') {
                ...
                if (e.shiftKey) {
                    ...
                    if (val.substring(lineStart).startsWith('  ')) {
                ...
                // Auto-indent matching current line indentation
                const start = textarea.selectionStart;
    ...
    // Initial load of schema and config
    (async () => {
        try {
            const r = await fetch('schema.json', {cache: 'no-cache'});
            if (r.ok) schemaData = await r.json();
        } catch (e) {}

        try {
            const r = await fetch('api/config', {cache: 'no-cache'});
            if (r.status === 410) {
                window.showToast('Config file is not set on the server.', 'error', 6000);
                ...
                window.showToast(`Error loading config: ${r.statusText} (${r.status})`, 'error', 6000);
            }
        } catch (e) {
            window.showToast(`Failed to load config: ${e.message}`, 'error', 6000);
        }
    })();

    // Save configuration
    saveBtn.addEventListener('click', async () => {
        const value = monacoEditor ? monacoEditor.getValue() : textarea.value;
        ...
        if (window.jsyaml) {
            try {
                ...
                window.showToast(`YAML Syntax Error: ${err.message}`, 'error', 7000);
                return;
            }
        }
        ...
        try {
            ...
            if (r.ok && dump !== await r.text()) {
                window.showToast('Config was changed from another place. Refresh the page and make changes again.', 'warning', 6000);
                saveBtn.disabled = false;
                saveBtn.innerHTML = originalHTML;
                return;
            }

            r = await fetch('api/config', {method: 'POST', body: value});
            if (r.ok) {
                dump = value;
                saveBtn.innerHTML = `
                    <span class="pulse-indicator"></span>
                ...
                try {
                    ...
                    // Server may close connection immediately upon exit
                }

                // Poll api/config for up to 12 seconds to verify the server successfully restarted
                let reconnected = false;
                for (let i = 0; i < 24; i++) {
                    await new Promise((res) => setTimeout(res, 500));
                    try {
                        const ping = await fetch('api/config', {cache: 'no-cache'});
                        if (ping.ok) {
                            reconnected = true;
                            break;
                        }
                ...
                `;
                if (reconnected) {
                    window.showToast('Configuration saved and server restarted successfully!', 'success');
                } else {
                    window.showToast('Configuration saved. Server restart signal sent.', 'info');
                }
                setTimeout(() => {
                    saveBtn.disabled = false;
                    saveBtn.innerHTML = originalHTML;
                ...
                window.showToast('Save error: ' + await r.text(), 'error', 6000);
                saveBtn.disabled = false;
                saveBtn.innerHTML = originalHTML;
            }
        } catch (err) {
            window.showToast(`Save failed: ${err.message}`, 'error', 6000);
            saveBtn.disabled = false;
            saveBtn.innerHTML = originalHTML;
        }
    });
    ...
    // Dynamic On-Demand Advanced Editor Loader
    if (btnLoadMonaco) {
        btnLoadMonaco.addEventListener('click', () => {
        if (monacoLoading || monacoEditor) return;
        monacoLoading = true;
        btnLoadMonaco.disabled = true;
        btnLoadMonaco.innerHTML = `
            <span class="pulse-indicator"></span>
            <span>Loading Advanced Editor...</span>
        `;
        if (loader) loader.classList.remove('hidden');

        const monacoCdn = 'https://cdn.jsdelivr.net/npm/monaco-editor@0.55.1/min';

        const script = document.createElement('script');
        script.src = `${monacoCdn}/vs/loader.js`;
        script.onload = () => {
            window.MonacoEnvironment = {
                getWorkerUrl: function () {
                    return `data:text/javascript;charset=utf-8,${encodeURIComponent(`
                        self.MonacoEnvironment = { baseUrl: '${monacoCdn}/' };
                        importScripts('${monacoCdn}/vs/base/worker/workerMain.js');
                    `)}`;
                }
            };

            require.config({paths: {vs: `${monacoCdn}/vs`}});
        ...
        script.onerror = () => {
            ...
            btnLoadMonaco.disabled = false;
            btnLoadMonaco.innerHTML = `<span>Retry Advanced Editor</span>`;
            window.showToast('Failed to load Advanced Editor from CDN. Check your internet connection.', 'error');
        };
    ...
    function initMonaco() {
        ...
        const getMonacoTheme = () => {
            const isDark = window.isDarkMode ? window.isDarkMode() : (document.documentElement.getAttribute('data-theme') !== 'light');
        ...
        btnLoadMonaco.innerHTML = `
            <svg viewBox="0 0 24 24"><path d="M9 16.2L4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4L9 16.2z"/></svg>
            <span>Advanced Editor Active</span>
        `;
        btnLoadMonaco.classList.add('badge-success');
        btnLoadMonaco.disabled = true;
